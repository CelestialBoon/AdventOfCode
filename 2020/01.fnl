...
(fn f1 [i j] 
    (local (good? result) (foo (. inputs i) (. inputs j)))
    (when good? (table.insert outputs result))
    (if (and (not good?) (< result 2020) (< j len))
        (f1 i (+ 1 j))
    (< i len)
        (f1 (+ 1 i) (+ 2 i))
        0
...
(fn f2 [i j k] 
    (local (good? result) (foo (. inputs i) (. inputs j) (. inputs k)))
    (when good? (table.insert outputs result))
    (if (and (not good?) (< result 2020) (< k len))
        (f2 i j (+ 1 k))
    (< j len)
        (f2 i (+ 1 j) (+ 2 j))
    (< i len)
